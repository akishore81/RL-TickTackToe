...
class TicTacToe():
    ...
    def is_winning(self, curr_state):
        ...
        if sum(curr_state[0:3]) == 15:
            return True
        elif sum(curr_state[3:6]) == 15:
            return True
        elif sum(curr_state[6:9]) == 15:
            return True
    ...
    def step(self, curr_state, curr_action):
        ...
        if status == 'Win':
            ...
            #env_state_action = self.action_space(Next_state)[1]
            #random_index = random.randrange(len(env_state_action))
            #random_action = env_state_action[random_index]
            
            random_action = random.choice(self.action_space(Next_state)[1])
            
            Update_Next_state = self.state_transition(Next_state,random_action)
            
            terminal, next_status = self.is_terminal(Update_Next_state)
            
            if next_status == 'Win':
                reward = -10
                return reward, Update_Next_state, terminal
            elif next_status == 'Tie':
                reward = 0
                return reward, Update_Next_state, terminal
            else:
                reward = -1
